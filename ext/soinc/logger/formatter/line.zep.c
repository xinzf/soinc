...
PHP_METHOD(Soinc_Logger_Formatter_Line, format) {
	...
	ZEPHIR_CALL_FUNCTION(&_2, "date", NULL, 37, &_1, date);
	...
	if (ZEPHIR_IS_FALSE(result)) {
		...
		ZEPHIR_CALL_METHOD(NULL, _6$$3, "__construct", NULL, 9, _8$$3);
		zephir_check_call_status();
	...
	ZEPHIR_CALL_FUNCTION(&replace0, "reset", NULL, 19, matches);
	ZEPHIR_UNREF(matches);
	zephir_check_call_status();
	ZEPHIR_MAKE_REF(matches);
	ZEPHIR_CALL_FUNCTION(&replace1, "end", NULL, 38, matches);
	ZEPHIR_UNREF(matches);
	zephir_check_call_status();
	...
	for (
	  ...
	  ; zephir_hash_move_forward_ex(_10, &_9)
		...
		if (zephir_array_isset_fetch(&val2, _12$$4, val1, 0 TSRMLS_CC)) {
			ZEPHIR_CALL_FUNCTION(&_13$$5, "strval", &_14, 39, val2);
			zephir_check_call_status();
	...
	ZEPHIR_CALL_FUNCTION(&_2, "array_values", NULL, 16, processors);
	zephir_check_call_status();
